...
import Debitor from '../models/Debitor.js';
...
router.get('/', protect, restrictTo('admin', 'cashier', 'sales_manager'), async (req, res) => {
  try {
    ...
    const [items, total] = await Promise.all([
      Debitor.find(query).sort({ createdAt: -1 }).skip(skip).limit(limit).lean(),
      Debitor.countDocuments(query)
    ]);

    res.json({ success: true, data: items, pagination: { page, limit, total, pages: Math.ceil(total / limit) } });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Не удалось получить список дебиторов' });
  }
});
...
router.post('/', protect, restrictTo('admin', 'cashier', 'sales_manager'), [
  ...
  try {
    const doc = await Debitor.create(req.body);
    res.status(201).json({ success: true, data: doc });
  } catch (err) {
    res.status(500).json({ success: false, message: 'Не удалось создать дебитора' });
  }
});
...
router.put('/:id', protect, restrictTo('admin', 'cashier'), [
  ...
  try {
    const updated = await Debitor.findByIdAndUpdate(req.params.id, req.body, { new: true, runValidators: true });
...
router.patch('/:id/status', protect, restrictTo('admin', 'cashier'), [
  ...
  try {
    const updated = await Debitor.findByIdAndUpdate(req.params.id, { status: req.body.status }, { new: true, runValidators: true });
    if (!updated) return res.status(404).json({ success: false, message: 'Запись не найдена' });
    res.json({ success: true, data: updated });
  } catch (err) {
...
// DELETE /api/debitors/:id (soft delete)
router.delete('/:id', protect, restrictTo('admin', 'cashier'), async (req, res) => {
  try {
    const doc = await Debitor.findById(req.params.id);

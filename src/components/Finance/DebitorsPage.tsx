...
import { DebitorRecord, LedgerStatus } from '../../types';

const statusOptions: LedgerStatus[] = ['В ожидании', 'Оплачено', 'Не оплачено'];

const DebitorsPage: React.FC = () => {
  const [items, setItems] = useState<DebitorRecord[]>([]);
  const [loading, setLoading] = useState(false);
  const [form, setForm] = useState<Partial<DebitorRecord>>({ currency: 'UZS', status: 'В ожидании' });
  const [filters, setFilters] = useState<{ status?: string; search?: string }>({});

  const canSubmit = useMemo(() => {
    return Boolean(form.partyName && form.amount && (form.amount as number) >= 0);
  }, [form.partyName, form.amount]);

  const load = async () => {
    setLoading(true);
    try {
      const res = await api.debitors.getAll({ limit: 100, status: filters.status, search: filters.search });
      if (res.success) {
  ...
  const handleCreate = async () => {
    ...
    const res = await api.debitors.create(payload);
  ...
  const handleUpdate = async (id: string, updates: Partial<DebitorRecord>) => {
    const res = await api.debitors.update(id, updates);
    if ((res as any).success) await load();
  };

  const handleDelete = async (id: string) => {
    if (!confirm('Bu kaydı silmek istediğinize emin misiniz?')) return;
    const res = await api.debitors.delete(id);
    if ((res as any).success) await load();
  };

  const handleStatusChange = async (id: string, status: LedgerStatus) => {
    const res = await api.debitors.updateStatus(id, status);
    if ((res as any).success) await load();
  };

  return (
    <div className="max-w-6xl mx-auto">
      <h2 className="text-2xl font-semibold mb-4">Дебиторы (Дебиторская задолженность)</h2>
      {/* Create form */}
